...
log_to_simmap <- function(dir_in, files, burnin=0.1){
  i=1
  for (file in files){
    path = paste0(dir_in, file)
    log <- read.table(path, header=TRUE) %>% select(char_hist)
    if(length(burnin)==1){
      log_discard <- round(burnin * length(log$char_hist), 0)
      log <- log %>% slice(log_discard+1:n())
    } else if (length(burnin)==length(files)){
      log_discard <- round(burnin[i] * length(log$char_hist), 0)
      log <- log %>% slice(log_discard+1:n())
    } else {
      cat("burnin should have length of 1 or ", length(files), ".")
    }
...
processSimplexParameters <- function(analysis_name, rf=FALSE, rates=FALSE){
  ...
  ind = 1
  for (i in all_dfs){
    df <- read_table(file = i, col_names = TRUE, progress = FALSE, show_col_types = FALSE)
    if (length(df)==0) next
    df <- as_tibble(df)
    if (rf){
      df <- df %>%
