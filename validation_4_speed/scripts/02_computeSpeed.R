...
grid = expand.grid(num_tips=num_tip, time_rb=NA,
...
for(i in 1:nrow(grid)) {
  ...
  history <- read.simmap(paste0(this_dir, "/character_histories/t",
                                this_tree, "_charhist.tree"),
                         format="phylip")
  
  cont <- read.nexus.data(paste0(this_dir, "/Continuous.nex")) %>% unlist()
...
write.csv(grid, file="output/r_pruning.csv")
...
for(i in 1:nrow(grid)) {
  ...
  # increment the progress bar
  setTxtProgressBar(bar, i / nrow(grißd))
}
